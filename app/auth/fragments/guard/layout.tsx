...
import RegisterFragment from "../register";
import VerifyEmailFragment from "../confirm";
import { ResetPassword, ForgotPassword } from "../password";
...
const Auth_Components: Record<AuthPage, React.FC> = {
    login: () => <Suspense fallback={<LoaderFragment />}><LoginFragment /></Suspense>,
    new: () => <Suspense fallback={<LoaderFragment />}><RegisterFragment /></Suspense>,
    confirm: () => <Suspense fallback={<LoaderFragment />}><VerifyEmailFragment /></Suspense>,
    forgot: () => <Suspense fallback={<LoaderFragment />}><ForgotPassword /></Suspense>,
    reset: () => <Suspense fallback={<LoaderFragment />}><ResetPassword /></Suspense>,
};

// ⬇️ Wrap `AuthorizeResourceHeaders` in Suspense to fix Next.js error
export default function AuthorizeResourceHeadersWrapper({ children }: Props) {
    return (
        <Suspense fallback={<LoaderFragment />}>
            <AuthorizeResourceHeaders>{children}</AuthorizeResourceHeaders>
        </Suspense>
    );
}

function AuthorizeResourceHeaders({ children }: Props) {
    const auth = useContext(AuthContext);
    const router = useRouter();
    const pathname = usePathname();
    const searchParams = useSearchParams(); // ⬅️ This is what requires Suspense
